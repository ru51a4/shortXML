        ...
        let buildXml = (node, isVariable = false) => {
            if (variable[node.variable] && !isVariable) {
                if (variable[node.variable]) {
                    resXML += `<${node.key} shortxml_print="false" shortxml_var="${node.variable}"/>`;
                }
                return;
            }
            if (node.variable) {
                ...
                if (node.isArray) {
                    resXML += `<${node.key} shortxml_print="true" shortxml_isarray="true" shortxml_var="${node.variable}">`;
                } else {
                    resXML += `<${node.key} shortxml_print="true" shortxml_var="${node.variable}">`;
                }
            } else {
                if (!node.isArray) {
                    resXML += "<" + node.key + ">";
    ...
    static decode(str) {
        ...
        superxmlparser74.parse(str,
            (item) => {
                ...
                let varPrint = item.attr.find((item) => item.key === "shortxml_print")?.value[0];
